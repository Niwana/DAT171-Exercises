...
def compute_path(predecessors, start, end):
    """ A function that converts a node predecessor matrix into a sequence of nodes. The
        predecessor matrix contains the previously visited node for each node in a series
        of jumps connecting the end node to the start node.

        The function starts at the end node and backtracks until it reaches the start node,
        then it reverses the list in order to return the desired path: start to end.

    :param predecessors: a predecessor matrix
    :param start: start node
    :param end: end node
    :return: the path of nodes connecting the start node to the end node
    """
    path = []
    current = end
    while current != start:
